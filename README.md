...
구성: 쇼핑몰 스타일 카드 그리드. 채널 탭(G마켓 신선 / G마켓 가공 / GS샵 /
카카오 톡딜 / NS몰)으로 전환하며, NS몰 탭은 농산 1~10 → 수산 1~10 → 축산 1~10
순서로 한 탭에 묶인다. 카드는 썸네일·순위 배지·상품명·정가(취소선)·할인율%·
판매가·평점/리뷰/구매(주문)수·AD/품절 배지를 표시한다. 데스크톱은 한 행 5개 ×
6줄(30개), 화면 폭에 따라 4/3/2열 반응형(모바일 대응). **📅 날짜 버튼**을 누르면
캘린더 모달이 열려 데이터가 있는 과거 일자를 골라 조회할 수 있고, 각 채널의 전체
스크린샷 저장본 링크도 제공한다. 모든 일자 데이터를 한 파일에 임베드해 외부 요청
없이 동작한다.

즉시 수집: main 에 `.recrawl` 파일을 변경해 푸시하면 crawl.yml 이 전체 타겟을
그 자리에서 수집(--all)하고 결과를 커밋·배포한다.
